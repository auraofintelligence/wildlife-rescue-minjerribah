...
test("keeps offline, GPS, real-map and safety features in the product source", async () => {
  ...
  assert.match(serviceWorker, /wrm-shell-v13/);
  ...
  assert.match(styles, /\.real-case-marker\s*\{[\s\S]*background:\s*#fff9ea/);
  assert.match(styles, /\.real-case-marker\.urgency-now\s*\{[\s\S]*border-color:\s*var\(--coral\)/);
  assert.match(styles, /\.real-case-marker\.urgency-watch\s*\{[\s\S]*border-color:\s*#4f8b58/);
